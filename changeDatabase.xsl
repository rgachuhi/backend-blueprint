...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="//ds:subsystem/ds:datasources/ds:datasource[@jndi-name='java:jboss/datasources/ExampleDS']">
        ...
        <ds:datasource jndi-name="java:jboss/datasources/ServiceDS" pool-name="ServiceDS" enabled="true"
                       use-java-context="true" use-ccm="true">
            <ds:connection-url>
                jdbc:mysql://${env.MYSQL_PORT_3306_TCP_ADDR}:${env.MYSQL_PORT_3306_TCP_PORT}/${env.MYSQL_DATABASE:template_db}
            </ds:connection-url>
            <ds:driver>mysql</ds:driver>
            <ds:security>
                <ds:user-name>${env.MYSQL_USER:template}</ds:user-name>
